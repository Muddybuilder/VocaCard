 ...
 * @author user
 */
public class SearchBoxFXMLController implements Initializable {
    @FXML
...
//		0x3163};
    ...
    char SBase = 0xAC00, LBase = 4352, VBase = 12623, TBase = 4519;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tfield = new TextField(){
            ...
            public void replaceText(int start, int end, String text) {
                ...
                if (text.matches(KeyList.getLangRegex(lang)))
                {
        ...
        tfield.addEventFilter(KeyEvent.KEY_PRESSED, (event)->{
            tfieldkey = true;
        ...
        tfield.addEventFilter(KeyEvent.KEY_RELEASED, (event)->{         
    ...
    private void handling(String code, boolean isReleased){
...
//                        System.out.println(dc);
        ...
        if (txt != null && !isReleased){
            String [][] deadkeys = KeyList.getKeyInfo(lang).deadkeys;
            if (prev.length() > 0){
                String mapped = getMappedValue(deadkeys, txt.getText(), prev);
                if (mapped.length() > 0){
                    //deleteAtCaret(1,0);
                ...
                if (isDeadKey(deadkeys, txt.getText())){
                    prev = txt.getText();
                }
                else{
                    if (lang == 26){
                        insertAtCaret(txt.getText());
    ...
    private void handleKeys(KeyEvent event, boolean isReleased){
        String code = event.getCode().toString();
